...
import { projects, departments, dashboardStats, formatCurrency, getStatusColor } from "@/lib/mock-data";
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="container -mt-12 relative z-10">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <StatCard icon={BarChart3} label="Total Projects" value={dashboardStats.totalProjects.toString()} sub="Across all departments" />
          <StatCard icon={CheckCircle2} label="Completed" value={dashboardStats.completedProjects.toString()} sub={`${dashboardStats.completionRate}% completion rate`} />
          <StatCard icon={TrendingUp} label="Total Budget" value={formatCurrency(dashboardStats.totalBudget)} sub={`${formatCurrency(dashboardStats.totalExpenditure)} spent`} />
          <StatCard icon={Clock} label="Ongoing" value={dashboardStats.ongoingProjects.toString()} sub={`${dashboardStats.planningProjects} in planning`} />
        </div>
      </section>
          ...
          {flagshipProjects.map((project, i) => (
            ...
            >
              ...
              <div className="mt-4 flex items-center justify-between text-xs">
                <span className="text-muted-foreground">Budget: <span className="font-medium text-foreground">{formatCurrency(project.budgetAllocation)}</span></span>
                <span className="text-muted-foreground">Spent: <span className="font-medium text-foreground">{formatCurrency(project.expenditure)}</span></span>
              </div>
            </div>
          ))}
        </div>
            ...
            {departments.map((dept) => (
              <div key={dept.id} className="rounded-xl border bg-card p-5 card-shadow hover:card-shadow-hover transition-all">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div>
                    <p className="text-xl font-bold text-foreground">{dept.projectCount}</p>
                    <p className="text-[11px] text-muted-foreground">Projects</p>
                  </div>
                  <div>
                    <p className="text-xl font-bold text-foreground">{formatCurrency(dept.totalBudget)}</p>
                    <p className="text-[11px] text-muted-foreground">Budget</p>
                  </div>
                  <div>
                    <p className="text-xl font-bold text-foreground">{Math.round((dept.totalExpenditure / dept.totalBudget) * 100)}%</p>
